...
# Обновленный рабочий адрес Stability AI Inpainting API
...
# Панель загрузки фото (необязательная)
uploaded_file = st.file_uploader("Загрузить фото для редактирования (опционально)", type=['png', 'jpg', 'jpeg'])

# Поле ввода текста
user_input = st.chat_input("Напиши сообщение или /inpainting [что сделать]")
...
if user_input:
    ...
    # Проверка: если это команда inpainting и загружено фото
    if user_input.startswith("/inpainting ") and uploaded_file:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.session_state.messages.append({
                        "role": "assistant", 
                        "content": f"Результат по запросу: {prompt_text}",
    ...
    # Если это просто общение (текст)
    elif not user_input.startswith("/inpainting"):
        with st.chat_message("assistant"):
            ...
            try:
                ...
                messages_for_llm.insert(0, {
                    "role": "system", 
                    "content": "Ты — Sused AI, крутой и дружелюбный ассистент. Твой создатель — Лёва."
                })
    ...
    elif user_input.startswith("/inpainting") and not uploaded_file:
        with st.chat_message("assistant"):
            st.warning("Чтобы использовать /inpainting, сначала загрузи картинку сверху!")
